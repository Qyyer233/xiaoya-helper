...
(function () {
    ...
    function main() {
        ...
        function evaluateTaskState(task, nowMs = Date.now()) {
            if (!task) return { state: 'unknown', remainingSec: 0, unlockAt: null };
            const now = nowMs;
            const endAt = parseDateTimeSafe(task.endTime);
            if (endAt && now >= endAt.getTime()) {
                return { state: 'expired', remainingSec: 0, unlockAt: null };
            }
            ...
            return { state: 'ready', remainingSec: 0, unlockAt };
        }
            ...
            if (isDangerZone) {
                ...
                GM_xmlhttpRequest({
                    ...
                    onload: function (response) {
                        try {
                            const match = response.responseText.match(/<title[^>]*>([\s\S]*?)<\/title>/i);
                            let title = match ? match[1] : "暂无公告";
                            title = title.replace(/\s+/g, ' ').replace(/\s*-\s*腾讯文档\s*$/i, '').trim();
                            const tempSpan = document.createElement('span');
                            tempSpan.innerHTML = title;
                            let finalContent = tempSpan.innerText;
                            finalContent = finalContent.replace(/【换行】/g, '\n');
        ...
        async function fetchTasksFromApi() {
            ...
            try {
                if (groupId) {
                    ...
                    if (res.success) {
                        const now2 = new Date();
                        taskQueue = res.data.filter(t => {
                            if (t.task_type !== 1) return false;
                            if (t.end_time && new Date(t.end_time) <= now2) return false;
                            return true;
                        }).map(t => ({
    ...
    `;
        ...
        function renderUI() {
            ...
            if (!isExpanded) {
                ...
                header.innerHTML = `<span class="xy-header-title">小雅自动刷<span class="xy-header-version">v1.3</span></span>`;
